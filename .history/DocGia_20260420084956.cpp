...
bool nhapThongTinDocGia(DocGia* dg) {
    if (!nhapTen("Nhap ho: ", dg->ho, 50)) return false;
    
    nhapTen("Nhap ten: ", dg->ten, 20);
    if (!nhapTen("Nhap ten: ", dg->ten, 20)) return false;
...
void nhapDocGia(QuanLyDocGia &ql) {
    ...
    if (nhapThongTinDocGia(docgia)) {
        if (themDocGia(ql, docgia)) {
            cout << "Them doc gia thanh cong!\n";
        }
    } else {
        cout << "Da huy them doc gia.\n";
        delete docgia;
    }
}
