...
void traCuuTheoTenNangCao(const ListDauSach &ds, const char* key) {
    ...
    char keyLower[100]; 
    strncpy(keyLower, key, sizeof(keyLower)-1); // lúc này keyLower đã được sao chép, với kích thước tối đa là 99 ký tự + 1 ký tự null terminator. Việc sử dụng sizeof(keyLower)-1 đảm bảo rằng chúng ta không ghi đè quá giới hạn của mảng keyLower, tránh được lỗi tràn bộ nhớ (buffer overflow) khi sao chép chuỗi từ key vào keyLower. nếu copy vào tầm 10 ký tự thì 
