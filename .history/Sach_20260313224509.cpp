...
void traCuuTheoTenNangCao(const ListDauSach &ds, const char* key) {
    ...
    char keyLower[100]; 
    strncpy(keyLower, key, sizeof(keyLower)-1); // sao chep key vao keyLower de giu nguyen key ban dau, tranh viec thay doi key khi toLowerCase se anh huong den cac thao tac khac sau nay neu co su dung lai key. -1 la chua cho 
    ...
    toLowerCase(keyLower); // chuyen keyLower ve chu thuong de so sanh khi tim kiem, giup cho viec tra cuu theo ten sach duoc linh hoat hon, khong phan biet chu hoa chu thuong trong qua trinh tra cuu.
