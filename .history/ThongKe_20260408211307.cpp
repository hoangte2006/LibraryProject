...
static void capNhatSoLuotMuonCuaDauSachDFS(TREE_DocGia root, ListDauSach &ds) {
    ...
    while (mt != nullptr) {
        DauSach* dau = nullptr;
        timSachTheoMa(ds, mt->maSach, dau);
...
void capNhatSoLuotMuonCuaDauSach(TREE_DocGia root, ListDauSach &ds) {
    // dùng for để reset về 0 trước, sau đó dùng DFS để cập nhật lại số lượt mượn cho từng đầu sách., mục đích reset để đảm bảo tính chính xác, tránh trường hợp có đầu sách nào đó bị thiếu lượt mượn do lỗi logic ở đâu đó trong quá trình cập nhật.
    for (int i = 0; i < ds.n; i++) {
