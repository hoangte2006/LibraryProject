...
void themCuonSach(DauSach* dauSach) { // NANG CAP: THEM HANG LOAT
    if (dauSach == nullptr) return; 
    ...
    char viTriGoiY[50];
    goiYViTriTiepTheo(dauSach, viTriGoiY);
    // Tao danh sach tam
    Sach** sachMoiArr = new Sach*[soLuong];
    int sttHienTai = laySTTMoi(dauSach);

    if (!nhapChuoiTuDo("Nhap vi tri: ", newSach->viTri, sizeof(newSach->viTri), viTriGoiY)) {
    ...
    cout << "Them cuon sach thanh cong!\n";
        ...
        if (key == 224) {
            ...
            showCursor(true);
            cout << "\nSua vi tri cho " << sachMoiArr[luaChon]->maSach << ": ";
            nhapChuoiTuDo("", sachMoiArr[luaChon]->viTri, sizeof(sachMoiArr[luaChon]->viTri), sachMoiArr[luaChon]->viTri);
            showCursor(false);
