...
void khoiTaoKhoMaThe(TREE_DocGia root) { 
    ...
    ifstream fileConfig("Input_file/config_ma.bin", ios::binary);
    if (fileConfig.is_open()) {
        fileConfig >> currentX;
        fileConfig.read((char*)&topIndex, sizeof(topIndex));
        fileConfig.read((char*)khoMa, sizeof(khoMa));
        fileConfig.close();
    } else {
        // lan chay dau tien khi chua co file config
        // ramdom bat ki bang thoi gian hien tai
        currentX = time(nullptr) % SO_LUONG_MA; 
        // Lan chay dau tien: tao mang tuan tu va xao tron (Fisher-Yates)
...
void luuKhoMaThe() {
    ...
    ofstream fileConfig("Input_file/config_ma.bin", ios::binary);
    if (fileConfig.is_open()) {
        fileConfig << currentX;
        fileConfig.write((char*)&topIndex, sizeof(topIndex));
        fileConfig.write((char*)khoMa, sizeof(khoMa));
        fileConfig.close();
    }
}

// Tao ma the moi bang LCG, tra ve -1 neu khong con ma nao trong
// Tao ma the moi O(1) tuyet doi bang ID Pool
int taoMaTheMoi(TREE_DocGia root) { 
    ...
    while (topIndex < SO_LUONG_MA) { 
        int maTheMoi = khoMa[topIndex++];
        // Kiem tra cay BST de du phong truong hop file config bi loi/xoa nhung data txt van con
