...
int laySTTMoi(DauSach* dauSach)
{
    ...
    const char* pos = strchr(dauSach->dsSach.pTail->maSach, '_'); // tim ki tu '_' trong maSach cuoi cung de lay so thu tu hien tai, sau do cong them 1 de tao maSach cho cuon sach moi.
    if (pos != nullptr) {
        return atoi(pos + 1) + 1;
    }
