...
// Thêm kiểm tra ngày hợp lệ trước khi chuyển đổi
time_t chuyenNgaySangTimeT(const Ngay& ngay) {
    if (!kiemTraNgayHopLe(ngay)) {
        // Có thể in ra lỗi hoặc trả về giá trị báo lỗi
        return (time_t)-1;
    }
    struct tm tmNgay;
    memset(&tmNgay, 0, sizeof(struct tm)); // Khởi tạo struct để tránh giá trị rác
    tmNgay.tm_sec = 0;
    tmNgay.tm_min = 0;
    tmNgay.tm_hour = 0;
    ...
    if (result == (time_t)-1) {
        cout << "Loi: Khong the chuyen doi ngay sang time_t.\n";
    }
    return result; 
}
...
int tinhSoNgay(const Ngay& ngayMuon, const Ngay& ngayHienTai) {
    ...
    if (timeMuon == (time_t)-1 || timeHienTai == (time_t)-1) {
        return -1; // Trả về lỗi nếu ngày không hợp lệ
    }
