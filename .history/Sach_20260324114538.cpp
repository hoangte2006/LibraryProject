...
void tinhSoNgay(const Ngay& ngayMuon, const Ngay& ngayHienTai) {
    // tinh so ngay giua ngayMuon va ngayHienTai, de biet duoc sach da muon bao lau, tu do co the xac dinh duoc sach co qua han hay khong.
    // ham nay se tinh so ngay giua 2 ngay
...
bool traSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayTra) {
    ...
    if (strcmp(muonTra->maSach, maSach) == 0 && muonTra->trangThai == 0) { // cho phep tra sach da bi mat (trangThai == 2) de cap nhat lai trang thai sach va muonTra, giup cho viec quan ly sach bi mat duoc hieu qua hon, va cung la de dam bao tinh chinh xac cua thong tin muon tra trong he thong quan ly thu vien.
