...
void khoiTaoKhoMaThe() { 
    ...
    if (fileConfig.is_open()) {
        // Chi doc 2 con so nho xiu (seed goc va vi tri dang boc)
    ...
    // Dung 1 ban sao cua seed de xao tron, nham giu nguyen currentSeed luu vao file
...
// Tao ma the moi bang cach boc the tu Pool
// Truyen root de kiem tra trung voi ma cu (lan dau chuyen doi tu he thong cu)
// Sau khi nextIdx da qua het ma cu, moi lan goi la O(1) tuyet doi
int taoMaTheMoi(TREE_DocGia root) {
    while (nextIdx < SO_LUONG_MA) {
        int ma = khoMa[nextIdx++];
        if (timDocGia(root, ma) == nullptr) return ma;
    }
    return -1;
