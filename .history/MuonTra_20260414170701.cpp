...
bool traSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayTra) {
    ...
    while (muonTra != nullptr) {
        if (strcmp(muonTra->maSach, maSach) == 0 && muonTra->trangThai == 0) { 
            ...
            // Kiem tra xem doc gia con sach nao qua han (hay bi mat) khong
            // Neu khong, va the dang bi khoa, thi tu dong mo khoa the
            if (docGia->trangThaiThe == 0) {
                bool vanConQuaHan = false;
                MuonTra* tempMT = docGia->dsMuonTra.pHead;
                while (tempMT != nullptr) {
                    if (tempMT->trangThai == 0 && tinhSoNgay(tempMT->ngayMuon, ngayTra) > HAN_MUON) {
                        vanConQuaHan = true;
                        break;
                    }
                    if (tempMT->trangThai == 2) { // sach bi mat
                        vanConQuaHan = true;
                        break;
                    }
                    tempMT = tempMT->pNext;
                ...
                if (!vanConQuaHan) {
                    ...
                    cout << "He thong da tu dong mo khoa the cho doc gia nay do khong con sach qua han/mat.\n";
                }
            }
