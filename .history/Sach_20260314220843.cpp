...
{
    if (s.top < MAX_SACH_MUON - 1) { // check lai dieu kien de tranh tran stack, neu stack da day thi khong the push them stt vao stack, dieu nay giup cho viec quan ly stack sttXoa duoc hieu qua hon, tranh cac van de lien quan den tran stack co the xay ra neu push them stt vao stack khi no da day.
...
bool baoMatSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach) {
    ...
    while (muonTra != nullptr) {
        if (strcmp(muonTra->maSach, maSach) == 0 && muonTra->trangThai == 0) {
            DauSach* dauSach = nullptr;
            Sach* sachBaoMat = timSachTheoMa(ds, maSach, dauSach);
            if (sachBaoMat != nullptr) {
                sachBaoMat->trangThai = 2; // cập nhật trạng thái thành "báo mất"
                cout << "Sach da duoc bao mat!\n";
                return true;
            } else {
                cout << "Khong tim thay sach trong he thong!\n";
                return false;
