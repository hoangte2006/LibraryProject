...
Ngay layNgayHienTai() {
    time_t t = time(nullptr); // 1/1/1970;
    struct tm* now = localtime(&t); // tinhs ra ngay thang nam hien tai.
    Ngay ngayHT;
    ngayHT.ngay = now->tm_mday;
    ngayHT.thang = now->tm_mon + 1; // +1 vi thang trong struct tm tinh tu 0;
    ngayHT.nam = now->tm_year + 1900; // +1900 vi nam trong struct tm tinh tu 1900;
    return ngayHT;
}

// Ham nay de chuyen doi ngay thang nam sang dang time_t (so giay tinh tu 1/1/1970)
// de tien hanh so sanh ngay thang nam, va tinh so ngay giua hai ngay thang nam khi can thiet. 
// Ham mktime se chuyen doi struct tm (chua thong tin ve ngay thang nam) sang dang time_t
// va chung ta co the su dung ket qua tra ve tu ham mktime de so sanh ngay thang nam, va tinh so ngay giua hai ngay thang nam khi can thiet.
time_t chuyenNgaySangTimeT(const Ngay& ngay) {
    ...
    return mktime(&tmNgay); // mktime se chuyen doi struct tm (chua thong tin ve ngay thang nam) sang dang time_t (so giay tinh tu 1/1/1970), va tra ve gia tri time_t do de chung ta co the su dung de so sanh ngay thang nam, va tinh so ngay giua hai ngay thang nam khi can thiet sau nay.
}
