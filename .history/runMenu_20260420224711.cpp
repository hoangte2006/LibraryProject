...
void thongKeUI(QuanLyDocGia& qlDocGia, ListDauSach& ds) {
    ...
    while (true) {
        ...
        if (key == 224 || key == 0) {
            key = _getch();
            if (key == 75) luaChon = (luaChon - 1 + 3) % 3; // LEFT  , công thức để chuyển vòng: (current - 1 + total) % total, ví dụ nếu current = 0 và total = 3, thì (0 - 1 + 3) % 3 = 2, tức là chuyển từ mục đầu tiên về mục cuối cùng
