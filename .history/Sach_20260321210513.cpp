...
bool traSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayTra) {
    ...
    if (dangMuonSach(docGia, maSach)) {
        cout << "Doc gia da muon cuon nay roi!\n";
        return false;
    }
    ...
    if (strcmp(muonTra->maSach, maSach) == 0 && (muonTra->trangThai == 0 || muonTra->trangThai == 2)) { // cho phep tra sach da bi mat (trangThai == 2) de cap nhat lai trang thai sach va muonTra, giup cho viec quan ly sach bi mat duoc hieu qua hon, va cung la de dam bao tinh chinh xac cua thong tin muon tra trong he thong quan ly thu vien.
...
{
    Sach* sach = timSachTheoMa(ds, maSach, *(new DauSach*)); // ham timSachTheoMa se tra ve con tro den cuon sach co maSach trung khop neu tim thay, va tra ve nullptr neu khong tim thay, nen khi goi ham timSachTheoMa de lay con tro den cuon sach can ghi nhan bi mat, chung ta co the truyen vao mot con tro tam thoi (new DauSach*) de nhan gia tri dau sach tim thay trong ham timSachTheoMa, de sau nay khi cap nhat trang thai sach do thanh 2 (thanh ly) thi cung cap nhat trang thai cua cuon sach do trong danh sach sach cua dau sach do duoc chinh xac va hieu qua hon, giup cho viec quan ly trang thai cua cac cuon sach trong he thong quan ly thu vien duoc tot hon.
    if (sach->trangThai != 1) {
        cout << "Sach khong o trang thai dang muon!\n";
        return false;
    }
