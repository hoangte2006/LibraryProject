...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach) {
    ...
    // --- TOI UU O(M): Gop 3 vong lap kiem tra thanh 1 vong lap duy nhat ---
    ...
    while (mt != nullptr) {
        if (mt->trangThai == 0) { // Chi xet cac sach dang muon
            soSachDangMuon++;
            ...
            // ngayMuon o tren dau ham da goi layNgayHienTai() roi
    ...
    if (soSachDangMuon >= MAX_SACH_MUON) {
        ...
        docGia->trangThaiThe = 0; // Khoa the doc gia
        return false;
    }       
