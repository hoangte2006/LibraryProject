...
void traCuuTheoTenNangCao(const ListDauSach &ds, const char* key) {
    ...
    char keyLower[100]; 
    strncpy(keyLower, key, sizeof(keyLower)-1); // sao chep key vao keyLower de giu nguyen key ban dau, tranh viec thay doi key khi toLowerCase se anh huong den cac thao tac khac sau nay neu co su dung lai key.
    keyLower[sizeof(keyLower)-1] = '\0'; //   Dam bao keyLower ket thuc bang ky tu null de tranh viec tràn bộ nhớ khi sử dụng sau này, đặc biệt là khi key có độ dài lớn hơn hoặc bằng kích thước của keyLower, điều này giúp đảm bảo tính an toàn và ổn định cho chương trình khi xử lý các chuỗi ký tự.
