...
// ham nay cho phep nhap ten hoac ho va ten, chi cho phep ky tu chu cai va khoang trang, tu dau moi tu viet hoa, khong cho phep 2 khoang trang lien tiep, khong cho phep khoang trang o dau va cuoi, va co the di chuyen con tro de sua loi khi dang nhap. Neu nhap sai thi se co am thanh va khong chap nhan ky tu do.
    ...
    while (true)
    {
        ...
        else if ((isalpha(c) || (isspace(c) && vt > 0 && result[vt-1] != ' ')) && len < maxLen - 1)
        {
            ...
            if (isalpha(charToInsert)) {
                charToInsert = (vt == 0 || result[vt - 1] == ' ') ? toupper(charToInsert) : tolower(charToInsert);
            }
