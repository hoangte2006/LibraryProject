...
int tinhQuaHanMax(const DocGia* docGia, const Ngay& ngayHienTai) {
    ...
    if (timeHienTai == (time_t)-1) return 0;
...
// Duyet cay DocGia thu thap phieu muon (cay DFS), tim DauSach qua Bang Bam Toan Cuc va cong luot muon
...
void capNhatSoLuotMuonCuaDauSach(TREE_DocGia root, ListDauSach &ds) {
    ...
    for (int i = 0; i < ds.n; i++) {
        if (ds.nodes[i] != nullptr) {
            ...
            int count = 0;
            Sach* current = ds.nodes[i]->dsSach.pHead;
            while (current != nullptr) {
                if (current->trangThai == 1) count++;
                current = current->pNext;
            }
            ds.nodes[i]->soLuotMuon = count; // Chi cap nhat du lieu dang duoc muon hien tai
        }
    }
