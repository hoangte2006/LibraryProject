...
bool xoaCuonSach(DauSach* dauSach, const char* maSach) {
    ...
    while (current != nullptr) {
        ...
        if (strcmp(current->maSach, maSach) == 0) {
            ...
            // ví dụ đã lấy được mã sách "ABC_0005", sau khi xóa cuốn sách này, chúng ta sẽ lấy được phần sau ký tự '_' là "0005", chuyển đổi sang số nguyên để lấy được số thứ tự 5, và sau đó đẩy số thứ tự này vào stack sttXoa để lưu lại số thứ tự của cuốn sách đã bị xóa, giúp cho việc tái sử dụng số thứ tự này khi thêm cuốn sách mới trong tương lai trở nên dễ dàng hơn, đồng thời giúp quản lý mã sách của các cuốn sách trong danh sách sách của đầu sách đó một cách hiệu quả hơn. 
            if (pos != nullptr)
            {
                int stt = atoi(pos + 1);
                pushSTTXoa(stt);
