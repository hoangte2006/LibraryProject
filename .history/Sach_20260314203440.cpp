...
bool xoaCuonSach(DauSach* dauSach, const char* maSach) {
    ...
    while (current != nullptr) {
        ...
        if (strcmp(current->maSach, maSach) == 0) {
            ...
            const char* pos = strchr(current->maSach, '_'); // tạo một con trỏ pos để tìm vị trí của ký tự '_' trong chuỗi maSach của cuốn sách hiện tại. Hàm strchr sẽ trả về con trỏ đến vị trí đầu tiên của ký tự '_' trong chuỗi maSach nếu tìm thấy, hoặc trả về nullptr nếu không tìm thấy ký tự đó. Việc tìm kiếm ký tự '_' là cần thiết để tách phần số thứ tự (STT) từ mã sách, vì mã sách được định dạng theo kiểu "ISBN_STT", và chúng ta cần lấy phần STT để đẩy vào stack sttXoa khi xóa cuốn sách này khỏi danh sách.
