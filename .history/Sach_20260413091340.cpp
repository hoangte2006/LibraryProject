...
// Vòng lặp này là lớp 2: Ràng buộc số trang phải > 0
    do {
        // Lớp 1 đã được giấu vào hàm: Chống nhập chữ
        p->soTrang = nhapSoNguyenHopLe("Nhap so trang (>0): "); 
        if (p->soTrang <= 0) cout << "So trang phai lon hon 0!\n";
    } while (p->soTrang <= 0);

    // Tương tự cho năm xuất bản
    do {
    ...
    cin.ignore(1000, '\n'); // Nhớ dọn nốt cái phím Enter trước khi đi tiếp
