...
void thongKeSoLuongTheoTheLoai(const ListDauSach &ds) {
    ...
    TheLoaiCount counts[MAX_THELOAI];
    int n = 0;

    for (int i = 0; i < ds.n; i++) {
        const char* tl = ds.nodes[i]->theLoai;

        int j;
        for (j = 0; j < n; j++) {
            if (strcmp(counts[j].theLoai, tl) == 0) {
                counts[j].count++;
                break;
            }
        }

        if (j == n && n < MAX_THELOAI) { 
            strcpy(counts[n].theLoai, tl);
            counts[n].count = 1;
            n++;
        }
    }
    ...
    for (int i = 0; i < n; i++) {
        cout << "The loai: " << counts[i].theLoai
             << " | So luong: " << counts[i].count
             << endl;
    }
}