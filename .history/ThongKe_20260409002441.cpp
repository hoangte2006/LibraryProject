...
// Bam chuoi ISBN thanh 1 so (vi tri trong bang bam)
...
// Dua DauSach vao bang bam, neu bi trung o thi tim o trong ke tiep
static void insertHash(HashEntry table[], DauSach* dsPtr) {
    int idx = hashString(dsPtr->ISBN);
    while (table[idx].daCoDuLieu) { 
        idx = (idx + 1) % HASH_SIZE;
    }
    int i = 0;
    ...
    table[idx].daCoDuLieu = true; 
}

// Tim nhanh con tro cua DauSach thong qua ma ISBN voi toc do O(1)
...
// Duyet cay DocGia thu thap phieu muon (cay DFS), tim DauSach qua Bang Bam va cong luot muon
...
// Ham chay chinh: Khoi tao Bang Bam -> Nap DauSach -> Quet DFS Cay Doc Gia -> Xoa Bang Bam
void capNhatSoLuotMuonCuaDauSach(TREE_DocGia root, ListDauSach &ds) {
    if (ds.n == 0) return;

    HashEntry* table = new HashEntry[HASH_SIZE]();
    // DONG FOR LAM NAY: Nap DauSach vao Bang Bam de tim kiem nhanh khi cap nhat so luot muon.
    for (int i = 0; i < ds.n; i++) {
