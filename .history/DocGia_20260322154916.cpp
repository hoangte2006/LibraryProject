...
void timMaxMaThe(TREE_DocGia root, int &maxID) {
    ...
    timMaxMaThe(root->pLeft, maxID);
    timMaxMaThe(root->pRight, maxID);
}
...
void sortDocGiaTheoTen(DocGia* arr[], int index) {
...
int partition(DocGia* arr[], int left, int right) {
    ...
    DocGia* pivotValue = arr[pivotIndex];
    swap(arr[pivotIndex], arr[right]); // di chuyen pivot ve cuoi de de dang phan chia

    int storeIndex = left;

    for (int i = left; i < right; i++) {
        if (soSanhTen(arr[i], pivotValue) < 0) {
            swap(arr[i], arr[storeIndex]);
            storeIndex++;
        }
    }

    swap(arr[storeIndex], arr[right]); // dat pivot ve vi tri chinh xac sau khi da phan chia
    return storeIndex;
