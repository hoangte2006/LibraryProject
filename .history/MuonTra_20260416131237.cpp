...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach) {
    ...
    while (mt != nullptr) {
        if (mt->trangThai == 0) { 
            ...
            if (tinhSoNgay(mt->ngayMuon, ngayDungLucMuon) > HAN_MUON) biQuaHan = true;// Qua han thi khong duoc muon tiep, cho du khong cung dau sach, vi vay dat len tren cung dau sach de uu tien kiem tra
        }
        mt = mt->pNext;
    }
