...
bool muonSach(DocGia* docGia, ListDauSach &ds, const char* maSach) {
    ...
    // --- KIEM TRA NGHEP TRONG 1 VONG LAP (Defensive Programming) ---
    if (docGia->soSachDangMuon >= MAX_SACH_MUON) {
        cout << "Doc gia da muon toi da " << MAX_SACH_MUON << " cuon sach!\n";
        return false;
    }
    ...
    while (mt != nullptr) {
        if (mt->trangThai == 0) { 
            ...
            if (tinhSoNgay(mt->ngayMuon, ngayDungLucMuon) > HAN_MUON) {
                cout << "Doc gia dang co sach muon qua han! Khong the muon.\n";
                return false;
            }
            
            // 2. Kiem tra trung dau sach
...
bool baoMatSach(DocGia* docGia, ListDauSach &ds, const char* maSach) {
    ...
    while (muonTra != nullptr) {
        if (strcmp(muonTra->maSach, maSach) == 0 && muonTra->trangThai == 0) {
            ...
            if (dauSach != nullptr) dauSach->soLuotMuon--;
            docGia->soSachDangMuon--;
...
bool traSachBiMat(DocGia* docGia, ListDauSach& ds, const char* maSach) {
    ...
    MuonTra* mt = docGia->dsMuonTra.pHead;
    while (mt != nullptr) {
        if (strcmp(mt->maSach, maSach) == 0 && mt->trangThai == 2) {
            DauSach* dauSach = nullptr;
            Sach* sach = timSachTheoMa(ds, maSach, dauSach);
            if (sach) sach->trangThai = 0; // Sach co the cho muon lai

            mt->trangThai = 1; // Ghi nhan da tra
            mt->ngayTra = layNgayHienTai();

            // Mo khoa neu khong con sach bi mat nao
            bool conSachMat = false;
            MuonTra* check = docGia->dsMuonTra.pHead;
            while (check != nullptr) {
                if (check->trangThai == 2) { conSachMat = true; break; }
                check = check->pNext;
            }
            if (!conSachMat) docGia->trangThaiThe = 1;

            cout << "Da hoan tra sach bi mat!";
            if (!conSachMat) cout << " The da duoc mo khoa.";
            cout << "\n";
            return true;
        }
        mt = mt->pNext;
    }
    cout << "Khong tim thay sach bi mat voi ma nay!\n";
    return false;
}
