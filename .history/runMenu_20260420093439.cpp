...
void quanLyDocGiaUI(QuanLyDocGia& ql) {
    ...
    while (true) {
        ...
        else if (key == 's' || key == 'S') { system("cls"); showCursor(true); hieuChinhDocGia(ql.root, displayArr[luaChon]->maThe); actionTaken = true; }
        else if (key == 't' || key == 'T') {
        ...
        while (inSub) {
            ...
            if (indexTrai == 1) { // --- QUAN LY DOC GIA ---
                ...
                if (chonPhai == 0) { system("cls"); showCursor(true); themDauSach(ds); }
                    ...
                    system("cls"); showCursor(true); 
                    char key[100]; 
                    if (nhapChuoiTuDo("Nhap ten sach can tim: ", key, 100)) {
