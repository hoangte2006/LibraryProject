...
int laySoThuTuLonNhat(const ListSach& ds)
{
    ...
    while (temp != nullptr)
    {
        const char* pos = strchr(temp->maSach, '_'); // tìm vị trí của ký tự '_' trong chuỗi maSach để tách phần số thứ tự ra khỏi mã sách. Hàm strchr sẽ trả về con trỏ đến vị trí của ký tự '_' trong chuỗi maSach, nếu tìm thấy, hoặc trả về nullptr nếu không tìm thấy. Việc tách phần số thứ tự từ mã sách là cần thiết để xác định số thứ tự lớn nhất hiện có trong danh sách sách của đầu sách, từ đó có thể tạo mã sách mới cho cuốn sách mới một cách chính xác và tránh trùng lặp mã sách.
...
void themCuonSach(DauSach* dauSach) {
    ...
    if (dauSach->dsSach.tongSoSach >= MAX_SACH_MUON) {
        cout << "Da dat toi da so luong cuon sach cho dau sach nay!\n";
        return;
    }
    ...
    sprintf(newSach->maSach,"%s_%04d", dauSach->ISBN, stt);
...
bool xoaCuonSach(DauSach* dauSach, const char* maSach) {
    ...
    while (current != nullptr) {
        ...
        if (strcmp(current->maSach, maSach) == 0) {
            ...
            int stt = atoi(strchr(current->maSach, '_') + 1);
            pushSTTXoa(stt);
