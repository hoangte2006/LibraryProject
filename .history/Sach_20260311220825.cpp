...
int laySoThuTuLonNhat(ListSach& ds)
{
    int maxSTT = 0;
    ...
    while (temp != nullptr)
    {
        // atoi la ham chuyen doi chuoi thanh so nguyen, va strchr(temp->maSach, '_') + 1 se tra ve con tro den ky tu ngay sau ky tu '_' trong maSach, do ma maSach duoc tao ra theo dinh dang "ISBN_STT", nen khi tach phan STT tu maSach, chung ta se lay phan sau ky tu '_' de chuyen doi sang so nguyen de tim so thu tu lon nhat hien co trong danh sach sach cua dau sach do.
        int stt = atoi(strchr(temp->maSach, '_') + 1);
...
void themCuonSach(DauSach* dauSach) {
    ...
    int stt = laySoThuTuLonNhat(dauSach->dsSach) + 1; 
