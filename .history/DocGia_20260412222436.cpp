...
void khoiTaoKhoMaThe(TREE_DocGia root) {
        cout << "[DEBUG] khoiTaoKhoMaThe - START" << endl;
    ...
    for (int i = count - 1; i > 0; i--) {
        int j = rand() % (i + 1); 
        int temp = maTheTrong[i];
        maTheTrong[i] = maTheTrong[j];
        maTheTrong[j] = temp;
    }
    
    topKho = -1;
    for (int i = 0; i < count; i++) {
        khoMaThe[++topKho] = maTheTrong[i];
    }
}
...
bool themDocGia(TREE_DocGia& root, DocGia* newnode) {
    cout << "[DEBUG] themDocGia: vao ham, newnode->maThe=" << (newnode ? newnode->maThe : -1) << ", root=" << root << endl;
    if (root == nullptr) {
        cout << "[DEBUG] themDocGia: root nullptr, gan newnode->maThe=" << newnode->maThe << endl;
        root = newnode;
        cout << "[DEBUG] themDocGia: gan thanh cong, root->maThe=" << root->maThe << endl;
        return true;
    }
    ...
    if (newnode->maThe == root->maThe) {
        ...
        delete newnode;
        cout << "[DEBUG] themDocGia: trung maThe, huy newnode, root->maThe=" << root->maThe << endl;
        return false;
    }       

    if (newnode->maThe < root->maThe) {
        cout << "[DEBUG] themDocGia: newnode->maThe < root->maThe, di sang trai" << endl;
        return themDocGia(root->pLeft, newnode);
    } else {
        cout << "[DEBUG] themDocGia: newnode->maThe > root->maThe, di sang phai" << endl;
        return themDocGia(root->pRight, newnode);
    }
    // End of function
    cout << "[DEBUG] themDocGia: ket thuc ham" << endl;
}
...
bool xoaDocGia(TREE_DocGia &root, int maThe) {
    ...
    if (maThe < root->maThe) {
        ...
        while (p != nullptr) {
            if (p->trangThai == 0) { 
                cout << "Khong the xoa doc gia nay vi van con sach chua tra!\n";
                return false;  
            }
            p = p->pNext; 
        }
