...
void BSTtoArray(TREE_DocGia root, DocGia* arr[], int &index) {
    ...
    if (index >= MAX_DOCGIA) return; // check an toan de tranh tran bo nho khi cay co so luong doc gia lon hon MAX_DOCGIA, neu index da vuot qua hoac bang MAX_DOCGIA thi se khong luu them dia chi cua node nao vao mang arr nua de tranh tran bo nho, nen tra ve de ket thuc ham.
