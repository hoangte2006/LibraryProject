...
// Sinh so ngau nhien trong [0, range-1] , range phai nho hon 2^32
...
static uint32_t xorshift32Range(uint32_t& state, uint32_t range) {
    // threshold = 2^32 % range: vi du range = 100 thi threshold = 2^32 % 100 = 56. Neu r < 56 thi r % 100 se phan bo khong deu (0-55 se xuat hien nhieu hon 56-99), nen ta se tiep tuc sinh r moi cho den khi r >= threshold de dam bao tinh phan bo deu.
...
// Tao ma the moi bang cach boc the tu Pool, toc do O(1) tuyt doie
int taoMaTheMoi() {
