...
bool dangMuonSach(const DocGia* docGia, const char* maSach) {
    ...
    while (temp != nullptr) {
        if (strcmp(temp->maSach, maSach) == 0 && temp->trangThai == 0) { // kiem tra neu ma sach trong thong tin muon tra trung khop voi ma sach can kiem tra, va trang thai cua thong tin muon tra do la dang muon (trangThai == 0), thi co nghia la doc gia dang muon cuon sach do, nen tra ve true de biet rang doc gia dang muon cuon sach do.  
            return true;
        }   
        temp = temp->pNext;
    }
...
// Ham duyet LNR lay dia chi cac node vao mảng.
