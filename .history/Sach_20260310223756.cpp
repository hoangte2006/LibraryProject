...
Sach* timSachTheoMa(const ListDauSach& ds, const char* maSach, DauSach*& dauSachTimThay)
{
    ...
    while (maSach[i] && !isdigit(maSach[i])) {
        isbn[i] = maSach[i];
        i++;
    }
    isbn[i] = '\0'; // Dam bao chuoi isbn ket thuc bang ky tu null

    // binary search de tim dau sach theo ISBN
    int left = 0;
    int right = ds.n - 1;

    while (left <= right) {
        int mid = left + ((right - left) >> 1); // >> 1 dich 1 bit, chia doi
        int cmp = strcmp(ds.nodes[mid]->ISBN, isbn);

        if (cmp == 0) {
            dauSachTimThay = ds.nodes[mid]; // bien dauSachTimThay se duoc gan gia tri cua dau sach tim thay, giup cho viec tra ve cuon sach sau nay de cap nhat trang thai khi muon sach.
            break;
        } else if (cmp < 0) {
            left = mid + 1;
        } else {
    ...
    return nullptr; // trả về nullptr nếu không tìm thấy cuốn sách có mã sách trùng khớp trong danh sách sách của đầu sách đã tìm thấy.
