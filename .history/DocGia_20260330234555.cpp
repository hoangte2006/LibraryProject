...
int tinhQuaHanMax(const DocGia* docGia) {
    if (docGia == nullptr) return 0;
    ...
    while (temp != nullptr) {
        if (temp->trangThai == 0) { // chi tinh qua han cho cac sach dang muon
            Ngay ngayMuon = temp->ngayMuon;
            Ngay ngayHienTai; // ham layNgayHienTai de lay ngay hien tai cua he thong, de so sanh voi ngayMuon de tinh so ngay qua han.
            layNgayHienTai(ngayHienTai);

            int soNgayQuaHan = tinhSoNgayQuaHan(ngayMuon, ngayHienTai); // ham tinhSoNgayQuaHan de tinh so ngay qua han tu ngayMuon den ngayHienTai, neu ngayHienTai nho hon ngayMuon thi tra ve 0, neu ngayHienTai lon hon ngayMuon thi tra ve so ngay qua han.
