...
void taoDuLieuGia(QuanLyDocGia& ql, ListDauSach& ds, int soLuong) {
    ...
    for (int i = 0; i < soLuong; i++) {
        ...
        bool daTaoSachQuaHan = false; // Flag de dam bao logic du lieu dung

        for (int j = 0; j < soSachMuon; j++) {
            if (daTaoSachQuaHan) break; // Neu da co sach qua han, khong cho muon them de du lieu hop le
            ...
            // Kiem tra xem sach vua tao co bi qua han so voi ngay hien tai khong
            if (tinhSoNgay(ngayMuonGia, layNgayHienTai()) > HAN_MUON) {
                daTaoSachQuaHan = true;
            }
