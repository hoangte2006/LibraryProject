...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach) {
    ...
    // Tach ISBN cua sach minh dinh muon (De phat hien da muon cung 1 Dau Sach chua)
    char isbnCanMuon[20];
    ...
    while (mt != nullptr) {
        if (mt->trangThai == 0) { 
            ...
            if (strcmp(isbnDangMuon, isbnCanMuon) == 0) { // Cung Dau Sach (ISBN) -> Khong duoc muon
                daMuonSachNay = true;
            }
            // Kiểm tra quá hạn: so sánh ngày mượn của từng sách với ngày hiện tại (chính là ngayMuon)
...
bool traSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach, Ngay ngayTra) {
    ...
    // 1. Tim phieu muon tuong ung cua doc gia (Tim cuc bo truoc de toi uu thoi gian)
    ...
    if (muonTra == nullptr) {
        cout << "Khong tim thay thong tin muon sach cho ma sach: " << maSach << " cua doc gia nay!\n";
        return false;
    }

    if (!soSanhNgay(muonTra->ngayMuon, ngayTra)) {
        cout << "Ngay tra khong hop le (Nho hon ngay muon)!\n";
        return false;
    }

    // 2. Sau khi xac nhan doc gia co muon sach do, moi tim den sach trong thu vien de cap nhat
    DauSach* dauSach = nullptr;
    Sach* sachTra = timSachTheoMa(ds, maSach, dauSach);

    if (sachTra == nullptr) {
        cout << "Loi du lieu: Khong tim thay sach nay trong he thong!\n";
        return false;
    }

    // 3. Cap nhat cac trang thai
    muonTra->trangThai = 1;
    muonTra->ngayTra = ngayTra;
    sachTra->trangThai = 0;

    // 4. Tu dong mo khoa the neu thoa dieu kien
    if (docGia->trangThaiThe == 0) {
        bool vanConQuaHan = false;
        MuonTra* tempMT = docGia->dsMuonTra.pHead;
        while (tempMT != nullptr) {
            // Neu co sach bi mat (2) hoac sach dang muon (0) nhung qua han thi khong the mo the
        ...
        if (!vanConQuaHan) {
            ...
            cout << "He thong da tu dong mo khoa the do doc gia khong con sach qua han hay lam mat.\n";
        }
    }
...
void xemSachDangMuon(const DocGia* docGia, const ListDauSach& ds) {
    ...
    while (temp != nullptr) {
        if (temp->trangThai == 0) { // Dang muon
            coSach = true;
            DauSach* dauSach = nullptr;
            // Ham timSachTheoMa dung de lay con tro den sach + dau sach,
            // nhung thu thuc ta chi can biet ten Dau Sach la du
