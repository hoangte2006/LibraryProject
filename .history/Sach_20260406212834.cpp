...
void hieuChinhDauSach(ListDauSach &ds) {
    ...
    // tai sao xoa va chen lai? vi ten sach thay doi co the lam mat di thu tu tang dan cua mang ds.nodes, nen can xoa di va chen lai vao vi tri moi cho dung thu tu tang dan.
    int i;
    for (i = 0; i < ds.n; i++) {
...
void hybridSortDauSach(DauSach* arr[], int left, int right) {
    ...
    // nếu mảng nhỏ → dùng insertion sort
