...
void toLowerCase(char* str) { 
    for (int i = 0; str[i]; i++) {
        str[i] = tolower((unsigned char)str[i]); // tranh viec truyen ky tu co dau sang ham to
    }
}
...
void traCuuTheoTenNangCao(const ListDauSach &ds, const char* key) {
    ...
    char keyLower[100]; 
    strncpy(keyLower, key, sizeof(keyLower)-1); // sao chep key vao keyLower de giu nguyen key ban dau, tranh viec thay doi key khi toLowerCase se anh huong den cac thao tac khac sau nay neu co su dung lai key. -1 la chua cho ky tu nul
