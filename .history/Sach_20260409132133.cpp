...
DauSach* timTheoISBN(const ListDauSach &ds, const char* ISBN) {
    ...
    int startIdx = idx;
    while (ds.hashTable[idx].daCoDuLieu) {
        if (!ds.hashTable[idx].daXoa && strcmp(ds.hashTable[idx].isbn, ISBN) == 0) { // strcmp(ds.hashTable[idx].isbn, ISBN) == 0 là điều kiện 
...
bool xoaDauSach(ListDauSach &ds, const char* ISBN) {
    for (int i = 0; i < ds.n; i++) {
        if (ds.nodes[i] != nullptr && strcmp(ds.nodes[i]->ISBN, ISBN) == 0) {
            ...
            xoaHashISBN(ds, ISBN); // O(1) xoa khoi bang bam
...
{ 
    ...
    // tim dau sach theo ISBN bang Hash Table O(1) 
