...
// O(1) tao ma the moi (Toan hoc thuan tuy)
int taoMaTheMoi(TREE_DocGia root) {
    int m = SO_LUONG_MA; // 90000
    int a = 61;
    int c = 17;
    ...
    while (true) {
        // Ap dung cong thuc toan hoc LCG
        currentX = (currentX * a + c) % m;
        
        // Cong them MIN_MA_THE (10000) de dich khoang ma ve [10000...99999]
        maTheMoi = currentX + MIN_MA_THE;
        
        // Kiem tra xem ma nay da ton tai trong cay chua
        if (timDocGia(root, maTheMoi) == nullptr) {
            break; // Neu chua ton tai thi lay ma nay!
        }
    }
...
bool xoaDocGia(TREE_DocGia &root, int maThe) {
    ...
    DocGia* target = timDocGia(root, maThe);
    if (target != nullptr) {
        if (target->dsMuonTra.pHead != nullptr) {
