...
Sach* timSachTheoMa(const ListDauSach& ds, const char* maSach, DauSach*& dauSachTimThay)
{
    ...
    while (maSach[i] != '_' && maSach[i] != '\0' && i < sizeof(isbn) - 1)  // sizeof(isbn) - 1 la de dam bao rang khi tach phan ISBN tu maSach, chuoi isbn duoc tao ra se khong vuot qua kich thuoc cua mang isbn, va con lai 1 ky tu de gan ky tu null
    {
        isbn[i] = maSach[i];
        i++;
    }
