...
bool muonSach(TREE_DocGia root, ListDauSach &ds, int maThe, const char* maSach) {
    ...
    // chổ này là chổ quan trọng nhất, vì nó liên quan đến việc cập nhật trạng thái của sách, cập nhật số lượng sách đang mượn của độc giả, và thêm thông tin mượn sách vào danh sách mượn trả của độc giả. Nếu có lỗi ở chỗ này, nó có thể dẫn đến việc dữ liệu không được cập
