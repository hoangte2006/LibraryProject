...
int chonTuBangDocGia(QuanLyDocGia& ql) {
    ...
    while (true) {
        ...
        gotoxy(0, 0); // Vẽ đè lên mà không dùng system("cls") để chống chớp chớp giật giật
    ...
    while (true) {
        ...
        for (int i = 0; i < ITEM_PER_PAGE; i++) {
            if (start + i < end) {
                ...
                string tenSach = ds.nodes[idx]->tenSach;
                if (tenSach.length() > 30) tenSach = tenSach.substr(0, 27) + "...";
                string tacGia = ds.nodes[idx]->tacGia;
                if (tacGia.length() > 20) tacGia = tacGia.substr(0, 17) + "...";

                cout << "│ " << left << setw(15) << ds.nodes[idx]->ISBN << " │ " << left << setw(30) << tenSach << " │ " << left << setw(20) << tacGia << " │ " << left << setw(15) << ds.nodes[idx]->theLoai << " │   ";
        ...
        setColor(36);
        cout << "   (Mui ten: Len/Xuong, PgUp/PgDn: Chuyen trang, Enter: Chon, ESC: Thoat)" << string(10, ' ') << "\n";
        resetColor();
...
void quanLySachUI(ListDauSach& ds) {
    ...
    while (true) {
        ...
        if (n == 0) {
            ...
            setColor(36); cout << "   (T) Them | (F) Tim | ESC: Quay lai\n"; resetColor();
        ...
        if (key == 0 || key == 224) {
            ...
            if (key == 72) luaChon = max(0, luaChon - 1);
                ...
                if (toupper(confirm) == 'Y') {
                    ...
                    gotoxy(0, 24); // Đẩy dòng thông báo lỗi mặc định xuống đáy màn hình
            ...
            // Tái sử dụng logic popup nút Delete để code gọn gàng
            ungetch(83); 
            ungetch(224); 
        }
        else if (key == 'c' || key == 'C') {
...
void quanLyDocGiaUI(QuanLyDocGia& ql) {
    ...
    while (true) {
        ...
        if (n == 0) {
            ...
            setColor(36);
            cout << "   (T) Them | (F) Tim | ESC: Quay lai\n";
            resetColor();
