...
static void quickSortISBN(DauSach* arr[], int left, int right) {
    if (left >= right) return;
    int mid = left + (right - left) / 2;
    DauSach* pivot = arr[mid];
    int i = left, j = right;
    
    while (i <= j) {
        while (strcmp(arr[i]->ISBN, pivot->ISBN) < 0) i++;
        while (strcmp(arr[j]->ISBN, pivot->ISBN) > 0) j--;
        if (i <= j) {
            DauSach* temp = arr[i];
            arr[i] = arr[j];
            arr[j] = temp;
    ...
    quickSortISBN(arr, left, j);
    quickSortISBN(arr, i, right);
}

static DauSach* binarySearchISBN(DauSach* arr[], int n, const char* isbn) {
    int left = 0, right = n - 1;
    while (left <= right) {
        int mid = left + (right - left) / 2;
        int cmp = strcmp(arr[mid]->ISBN, isbn);
...
static void capNhatSoLuotMuonCuaDauSachDFS(TREE_DocGia root, DauSach* arrByISBN[], int nDauSach) {
    if (root == nullptr) return;

    capNhatSoLuotMuonCuaDauSachDFS(root->pLeft, arrByISBN, nDauSach);
    ...
    while (mt != nullptr) {
        ...
        DauSach* dau = binarySearchISBN(arrByISBN, nDauSach, isbn);
    ...
    capNhatSoLuotMuonCuaDauSachDFS(root->pRight, arrByISBN, nDauSach);
}

void capNhatSoLuotMuonCuaDauSach(TREE_DocGia root, ListDauSach &ds) {
    if (ds.n == 0) return;

    DauSach** arrByISBN = new DauSach*[ds.n];
    for (int i = 0; i < ds.n; i++) {
        if (ds.nodes[i] != nullptr) {
            ds.nodes[i]->soLuotMuon = 0;
            arrByISBN[i] = ds.nodes[i];
        }
    }

    quickSortISBN(arrByISBN, 0, ds.n - 1);

    capNhatSoLuotMuonCuaDauSachDFS(root, arrByISBN, ds.n);
