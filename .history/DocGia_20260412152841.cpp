...
// Ham tao ma the moi - Random ngau nhien de chong suy bien cay BST
// --- CHIEN THUAT KHO MA THE (Loc va Xao) ---
static int khoMaThe[9000];
static int topKho = -1;
static bool usedMaThe[10000]; 

// Ham danh dau cac ma the da ton tai trong cay BST vao mang usedMaThe de loc ra cac ma the con trong kho ma the.
...
void khoiTaoKhoMaThe(TREE_DocGia root) {
    for (int i = 0; i < 10000; i++) usedMaThe[i] = false;
    danhDauMaThe(root);
    
    int availableCodes[9000];
    int count = 0
    for (int i = 1000; i <= 9999; i++) {
        if (!usedMaThe[i]) {
            availableCodes[count++] = i;
        }
    }
    
    for (int i = count - 1; i > 0; i--) {
        int j = rand() % (i + 1);
        int temp = availableCodes[i];
        availableCodes[i] = availableCodes[j];
        availableCodes[j] = temp;
    }
    
    topKho = -1;
    for (int i = 0; i < count; i++) {
        khoMaThe[++topKho] = availableCodes[i];
    }
}
