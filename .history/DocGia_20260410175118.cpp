...
int taoMaTheMoi(TREE_DocGia root) {
    ...
    do {
        ...
        biTrung = (timDocGia(root, maTheMoi) != nullptr); // Kiểm tra xem ma the đã tồn tại chưa, nếu tồn tại thì biTrung sẽ là true và vòng lặp sẽ tiếp tục tạo mã mới cho đến khi tìm được mã không trùng lặp.
