...
void runMenu() {
    ...
    // --- DONG BO DU LIEU (FIX LOI "BONG MA") ---
    // Reset lai toan bo so luot muon va trang thai sach ve mac dinh
    for (int i = 0; i < ds.n; i++) {
        ds.nodes[i]->soLuotMuon = 0;
        Sach* s = ds.nodes[i]->dsSach.pHead;
        while (s != nullptr) {
            if (s->trangThai == 1) s->trangThai = 0; // Chi mo khoa cac sach bi treo trang thai "Da Muon"
            s = s->pNext;
        }
    }
    // Quet lai toan bo lich su muon/tra hien co cua DocGia de phuc hoi trang thai that
