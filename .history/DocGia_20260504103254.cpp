...
//  KHO CHUA MA THE (ID POOL + DE QUY)

static int khoMa[SO_LUONG_MA];
static int nextIdx = 0;
static uint32_t currentSeed = 1;

// ── THUAT TOAN CU (XORSHIFT32) - GIU LAI DE THAM KHAO ──────────────────────
// // Thuat toan Xorshift32: Sieu nhanh, 2^32 so
...
// }
//
// // Sinh so ngau nhien trong [0, range-1], dam bao phan bo deu bang rejection sampling
...
// }
//
// // Xao tron Fisher-Yates bang Xorshift32 (vong for)
...
// Xao tron mang bang de quy (Fisher-Yates phien ban de quy)
// Moi vong: sinh so ngau nhien j trong [0, n-1], co dinh arr[n-1], de quy n-1 phan tu con lai
static void xaoTronDeQuy(int arr[], int n, uint32_t& seed) {
    if (n <= 1) return;                         
    seed = seed * 1103515245 + 12345;           // (cong thuc LCG)
    int j = seed % n;                           
    int temp = arr[n-1]; arr[n-1] = arr[j]; arr[j] = temp;
    xaoTronDeQuy(arr, n - 1, seed);             
// Xao tron mang bang vong lap (Fisher-Yates phien ban lap) de tranh Stack Overflow
static void xaoTronVongLap(int arr[], int n, uint32_t& seed) {
    for (int i = n; i > 1; i--) {
        seed = seed * 1103515245 + 12345;           // (cong thuc LCG)
        int j = seed % i;                           
        int temp = arr[i - 1]; 
...
void khoiTaoKhoMaThe() {
    ...
    // 2. Xao tron bang de quy
    // 2. Xao tron bang vong lap
    // Dung ban sao cua seed de xao tron, giu nguyen currentSeed luu vao file
    uint32_t tempSeed = currentSeed;
    xaoTronDeQuy(khoMa, SO_LUONG_MA, tempSeed);
    xaoTronVongLap(khoMa, SO_LUONG_MA, tempSeed);
}
