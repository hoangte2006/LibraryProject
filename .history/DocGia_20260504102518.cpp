...
static void xaoTronDeQuy(int arr[], int n, uint32_t& seed) {
    if (n <= 1) return;                         
    seed = seed * 1103515245 + 12345;           // sinh so ngau nhien moi (cong thuc LCG)
    int j = seed % n;                           // chon vi tri ngau nhien trong [0, n-1]
    int temp = arr[n-1]; arr[n-1] = arr[j]; arr[j] = temp; // hoan vi
    xaoTronDeQuy(arr, n - 1, seed);             // de quy n-1 phan tu con lai
}
