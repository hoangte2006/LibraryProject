...
Sach* timSachTheoMa(const ListDauSach& ds, const char* maSach, DauSach*& dauSachTimThay)
{
    ...
    isbn[i] = '\0'; // Thao tac tren de tach ISBN từ mã sách, giả sử mã sách có định dạng "ISBN12345", đoạn mã này sẽ trích xuất phần "ISBN" và lưu vào biến isbn, giúp chúng ta có thể tìm kiếm đầu sách dựa trên mã ISBN được tách ra từ mã sách.
}
