...
dsNew->dsSach.pHead = dsNew->dsSach.pTail = nullptr; 
        ...
        // BẮT ĐẦU FIX LOGIC SORT: Tìm vị trí chèn để mảng luôn tăng dần theo tên
        int pos = timViTriChen(ds, dsNew->tenSach); 

        // Dời các phần tử phía sau ra sau 1 bước để lấy chỗ trống
        for (int i = ds.n; i > pos; i--) {
            ds.nodes[i] = ds.nodes[i - 1];
        }
        
        // Chèn đầu sách vào đúng vị trí và tăng số lượng
        ds.nodes[pos] = dsNew;
        ds.n++;

        insertHashISBN(ds, dsNew); // O(1) chen vao bang bam        getline(file, line); // #BOOK
