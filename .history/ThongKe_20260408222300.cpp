...
static unsigned long hashString(const char* str) {
    ...
    // Duyệt qua từng ký tự cho đến khi gặp ký tự kết thúc '\0'
    for (int i = 0; str[i] != '\0'; i++) {
        int c = (int)str[i]; // Lấy mã ASCII của ký tự hiện tại
        
        // Công thức gốc: hash = hash * 33 + c
        // Viết kiểu dịch bit cho chuyên nghiệp và nhanh:
        hash = ((hash << 5) + hash) + c; 
    }
    
    // Ép kết quả vào phạm vi ngăn tủ từ 0 đến 2002
