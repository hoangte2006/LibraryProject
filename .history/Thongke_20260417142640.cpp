...
static void capNhatSoLuotMuonCuaDauSachDFS(TREE_DocGia root, ListDauSach &ds) {
    ...
    // Xử lý nút hiện tại (Độc giả)
    MuonTra* mt = root->dsMuonTra.pHead;
    while (mt != nullptr) {
        // Nếu chỉ đếm sách ĐANG mượn (trạng thái = 0)
        // Nếu muốn đếm TẤT CẢ lượt mượn, hãy cân nhắc bỏ điều kiện if này
        if (mt->trangThai == 0) { 
            char isbn[20];
        ...
        mt = mt->pNext; // Luôn tiến tới nút tiếp theo, tránh loop vô tận
    }
