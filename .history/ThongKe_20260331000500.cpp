...
void lietKeDocGiaQuaHan(TREE_DocGia root) {
    ...
    //
    for (int i = 0; i < countQH - 1; i++) {
        for (int j = 0; j < countQH - i - 1; j++) {
            if (listQH[j].soNgayQuaHan < listQH[j + 1].soNgayQuaHan) {
                DocGiaQuaHan temp = listQH[j];
                listQH[j] = listQH[j + 1];
                listQH[j + 1] = temp;
            }
        }
    ...
    cuot << "--- DANH SACH DOC GIA QUA HAN ---\n";
