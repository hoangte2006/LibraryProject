    ...
    while (current != nullptr) {
        if (strcmp(current->maSach, maSach) == 0) {
            ...
            if (current == dauSach->dsSach.pHead) {
                ...
                // Nếu xóa node ở giữa hoặc cuối
                prev->pNext = current->pNext; // Cho node trước đó trỏ thẳng đến node sau nó
            }

            // Cập nhật lại con trỏ pTail nếu xóa node cuối
            if (current == dauSach->dsSach.pTail) {
                dauSach->dsSach.pTail = prev;
            }
