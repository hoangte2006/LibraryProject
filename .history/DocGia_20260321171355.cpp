...
bool dangMuonSach(const DocGia* docGia, const char* maSach) {
    ...
    while (temp != nullptr) {
        if (strcmp(temp->maSach, maSach) == 0 && temp->trangThai == 0) {
            return true; // trả về true nếu tìm thấy mã sách trùng khớp và trạng thái mượn là 0 (đang mượn).
        }
        temp = temp->pNext;
    }

    return false; // trả về false nếu không tìm thấy mã sách trùng khớp hoặc tất cả các giao dịch mượn có mã sách trùng khớp đều đã được trả.
}
