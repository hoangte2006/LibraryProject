...
DocGia* thucHienXoa(TREE_DocGia root, int maThe, bool &isDeleted) {
    ...
    if (maThe < root->maThe) {
        ...
        // TH3: Node co 2 con
            ...
            // Tim node the mang (Successor): Node nho nhat ben phai
            DocGia* successor = root->pRight;
            while (successor->pLeft != nullptr) {
                successor = successor->pLeft;
            }

            // Copy thong tin tu successor sang node hien tai (Cach nay la chuan va de hieu nhat)
            ...
            root->pRight = thucHienXoa(root->pRight, successor->maThe, isDeleted);
            isDeleted = true; // Dam bao van tinh la da xoa thanh cong
        }
    }
    return root;
}
