...
void capNhatTrangThaiSachDangMuon(TREE_DocGia root, ListDauSach& ds)
{
    ...
    while (mt != nullptr)
    {
        if (mt->trangThai == 0) 
        {
            ...
            if (sach != nullptr) // để dam bao rang chi cap nhat trang thai cua sach khi tim thay cuon sach co maSach trung khop voi maSach trong thong tin muon tra cua doc gia, de tranh truong hop co thong tin muon tra cua doc gia ma maSach khong ton tai trong danh sach dau sach, neu khong check dieu kien nay thi se co nguy co bi loi khi cap nhat trang thai cua sach do khi tim khong thay cuon sach do trong danh sach dau sach va sau do truy cap vao con tro nullptr de cap nhat trang thai cua sach do, dieu nay               
            {
                sach->trangThai = 1; 
            }
        }
