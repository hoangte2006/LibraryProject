    ...
    if (maThe < root->maThe) {
        ...
        // quy tat don gian: chon X nho nhat trong nhanh phai node 3 nhung lon lon node 3.
            ...
            if (replacementParent == root) {
                root->pRight = replacement->pRight;
            } else {
                replacementParent->pLeft = replacement->pRight;
            }

            root->maThe = replacement->maThe;
            strcpy(root->ho, replacement->ho);
            strcpy(root->ten, replacement->ten);
            strcpy(root->giotinh, replacement->giotinh);
            root->trangThaiThe = replacement->trangThaiThe;
