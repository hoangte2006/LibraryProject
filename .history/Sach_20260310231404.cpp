...
void themCuonSach(DauSach* dauSach) {
    ...
    sprintf(newSach->maSach,"%s_%04d", dauSach->ISBN, dauSach->dsSach.tongSoSach + 1);

    cout << "Ma sach duoc tao: " << newSach->maSach << endl; // in ra mã sách đã được tạo tự động dựa trên mã ISBN của đầu sách và số lượng cuốn sách hiện có của đầu sách đó, giúp người dùng biết được mã sách của cuốn sách mới mà họ vừa thêm vào danh sách sách của đầu sách hiện tại.
