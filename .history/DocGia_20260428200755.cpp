...
// --- KHO CHUA MA THE (ID POOL + FREE STACK + MT19937) ---
...
// Stack tai su dung ma the khi xoa doc gia (O(1))
static int freeStack[MAX_DOCGIA + 10];
static int topFree = -1;

void traLaiMaThe(int maThe) {
    freeStack[++topFree] = maThe;
}
...
void khoiTaoKhoMaThe(TREE_DocGia root) { 
    ...
    if (fileConfig.is_open()) {
        ...
        int sz;
        if (fileConfig >> sz) {
            for (int i = 0; i < sz; i++) fileConfig >> freeStack[i];
            topFree = sz - 1;
        }
        fileConfig.close();
    } else {
    ...
    // 1. Tao mang tuan tu
    for (int i = 0; i < SO_LUONG_MA; i++) khoMa[i] = MIN_MA_THE + i;

    // 2. Xao tron mang bang thuat toan Mersenne Twister (Chuan C++11)
    // Thay the hoan toan rand() de khong bi gioi han RAND_MAX = 32767
    mt19937 rng(currentSeed);
...
void luuKhoMaThe() {
    ...
    if (fileConfig.is_open()) {
        fileConfig << currentX;
        fileConfig << currentSeed << " " << nextIdx << "\n";
        fileConfig << (topFree + 1) << "\n";
        for (int i = 0; i <= topFree; i++) fileConfig << freeStack[i] << " ";
        fileConfig.close();
    }
}

// Tao ma the moi bang LCG, tra ve -1 neu khong con ma nao trong
// Tao ma the moi O(1) thuc su
int taoMaTheMoi(TREE_DocGia root) { 
    ...
    // Uu tien lay ma da thu hoi trong Free Stack ra dung truoc
    if (topFree >= 0) return freeStack[topFree--];
    // Neu khong, boc tu Pool goc
...
bool xoaDocGia(QuanLyDocGia &ql, int maThe) {
    if (xoaDocGiaHelper(ql.root, maThe)) {
        ql.soLuongDocGia--;
        traLaiMaThe(maThe); // Tra lai ma vao Stack de tai su dung
        return true;
    }
    return false;
}
