...
void themCuonSach(DauSach* dauSach) {
    ...
    int stt = laySTTMoi(dauSach->dsSach);
...
bool xoaCuonSach(DauSach* dauSach, const char* maSach) {
    ...
    while (current != nullptr) {
        ...
        if (strcmp(current->maSach, maSach) == 0) {
            ...
            const char* pos = strchr(current->maSach, '_'); // tra ve con tro den ky tu '_' trong maSach de tach phan STT ra de dua vao stack sttXoa, giup cho viec tai su dung lai cac STT da bi xoa khi them cuon sach moi sau nay, dieu nay giup cho viec quan ly maSach duoc don gian hon va tranh viec tao ra cac maSach co STT lon khong can thiet khi them cuon sach moi sau khi da xoa nhieu cuon sach trong danh sach sach cua dau sach do. Việc sử dụng strchr giúp chúng ta dễ dàng tìm kiếm vị trí của ký tự '_' trong maSach để tách phần STT ra một cách chính xác và hiệu quả, đồng thời giúp duy trì tính nhất quán trong việc quản lý mã sách của các cuốn sách trong hệ thống quản lý thư viện.
...
{ 
    ...
    for (int j = 0; j < ds.n; j++) {
        if (ds.nodes[j] != nullptr && strcmp(ds.nodes[j]->ISBN, isbn) == 0) {
    ...
    while (temp != nullptr) {
        if (strcmp(temp->maSach, maSach) == 0) {
            return temp; // trả về con trỏ đến cuốn sách có mã sách trùng khớp nếu tìm thấy.
        }
        temp = temp->pNext; 
...
void pushSTTXoa(int stt) {
    if (sttXoa.top < 999) {
        sttXoa.data[++sttXoa.top] = stt;
    }   
}

int laySTTMoi(ListSach &ds) {
    if (sttXoa.top >= 0) { 
        return sttXoa.data[sttXoa.top--];
    } else {
        return laySoThuTuLonNhat(ds) + 1;
        // +1 la de tang stt len cho cuon sach moi, vi stt duoc tao ra theo dinh dang "ISBN_STT", nen khi tach phan STT tu maSach, chung ta se lay phan sau ky tu '_' de chuyen doi sang so nguyen de tim so thu tu lon nhat hien co trong danh sach sach cua dau sach do, va khi them cuon sach moi, chung ta se tang stt len 1 don vi de tao maSach cho cuon sach moi theo dinh dang "ISBN_STT".
    }
}
