...
// Ham xoaDocGia da duoc toi uu so lan so sanh va kiem tra lich su muon tra
bool xoaDocGia(QuanLyDocGia &ql, int maThe) {
    TREE_DocGia* current = &(ql.root);
    
    while (*current != nullptr) {
        if (maThe == (*current)->maThe) {
            // Tim thay node, kiem tra dieu kien xoa
            if ((*current)->dsMuonTra.pHead != nullptr) {
                cout << "Loi: Khong the xoa! Doc gia nay da co lich su giao dich.\n";
                return false;
            }

            DocGia* nodeToDelete = *current;
            
            // TH1: Node la hoac chi co 1 con
            if (nodeToDelete->pLeft == nullptr) {
                *current = nodeToDelete->pRight;
            } else if (nodeToDelete->pRight == nullptr) {
                *current = nodeToDelete->pLeft;
            } else {
                // TH2: Node co 2 con - Tim node the mang (min cua cay con phai)
                DocGia** replacement = &(nodeToDelete->pRight);
                while ((*replacement)->pLeft != nullptr) {
                    replacement = &((*replacement)->pLeft);
                }
                
                DocGia* temp = *replacement;
                *replacement = temp->pRight; // Tach node the mang ra
                
                // Thay the node hien tai bang node the mang
                temp->pLeft = nodeToDelete->pLeft;
                temp->pRight = nodeToDelete->pRight;
                *current = temp;
            }

            giaiPhongMuonTra(nodeToDelete->dsMuonTra);
            delete nodeToDelete;
            ql.soLuongDocGia--;
            return true;
        }
