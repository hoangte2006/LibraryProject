...
// làm hash table cho dau sach de khi duyet ds muon tra cua doc gia thi co the nhanh chong cap nhat so luot muon cho dau sach do, thay vi phai tim kiem tu dau sach -> ds sach -> ma sach moi cap nhat duoc, dieu nay se lam giam thoi gian cap nhat so luot muon khi co nhieu doc gia va dau sach.
...
static void insertHash(HashEntry table[], DauSach* dsPtr) {
    ...
    while (table[idx].daCoDuLieu) { // Nếu ô này đã có dữ liệu, nhích qua ô tiếp theo tìm chỗ trống
        idx = (idx + 1) % HASH_SIZE;
    }
    int i = 0;
    ...
    table[idx].daCoDuLieu = true; // Lưu xong thì bật cờ báo hiệu đã có dữ liệu
}

static DauSach* searchHash(HashEntry table[], const char* isbn) {
    int idx = hashString(isbn);
    int startIdx = idx;
    while (table[idx].daCoDuLieu) { // Chỉ tra cứu trong những ô đã có dữ liệu
