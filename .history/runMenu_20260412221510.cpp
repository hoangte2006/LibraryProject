...
void runMenu()
{
    ...
    cout << "[DEBUG] Bat dau loadDauSach" << endl;
    loadDauSach("Input_file/DauSach.txt", ds);
    cout << "[DEBUG] Da xong loadDauSach" << endl;
    loadDocGia("Input_file/DocGia.txt", root);
    cout << "[DEBUG] Da xong loadDocGia" << endl;
    // Debug: kiểm tra cây DocGia
    cout << "[DEBUG] root address: " << root << endl;
    if (root) {
        cout << "[DEBUG] root->pLeft: " << root->pLeft << ", root->pRight: " << root->pRight << endl;
    }
    // Đếm số node DocGia
    int countDocGia = 0;
    std::function<void(TREE_DocGia)> countNodes = [&](TREE_DocGia node) {
        if (!node) return;
        countDocGia++;
        countNodes(node->pLeft);
        countNodes(node->pRight);
    };
    countNodes(root);
    cout << "[DEBUG] So node DocGia: " << countDocGia << endl;
    // Kiểm tra số lượng dau sach
    cout << "[DEBUG] So dau sach: " << ds.n << endl;
    cout << "[DEBUG] Goi capNhatTrangThaiSachDangMuon" << endl;
    capNhatTrangThaiSachDangMuon(root, ds);
    cout << "[DEBUG] Xong capNhatTrangThaiSachDangMuon" << endl;
    cout << "[DEBUG] Goi capNhatSoLuotMuonCuaDauSach" << endl;
    capNhatSoLuotMuonCuaDauSach(root, ds);
    cout << "[DEBUG] Xong capNhatSoLuotMuonCuaDauSach" << endl;
    cout << "[DEBUG] Goi khoiTaoKhoMaThe" << endl;
    khoiTaoKhoMaThe(root); // Khoi tao Stack id sau khi da load data
    cout << "[DEBUG] Xong khoiTaoKhoMaThe" << endl;
