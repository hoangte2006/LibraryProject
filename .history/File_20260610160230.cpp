...
// --- THUAT TOAN HO TRO BULK LOADING (O(N log N)) ---
static int soSanhTenSachLoad(DauSach* a, DauSach* b) {
    return stricmp(a->tenSach, b->tenSach);
}

static void swapDauSachLoad(DauSach* &a, DauSach* &b) {
...
void ghiDocGia(ofstream &file, TREE_DocGia root)
{
    ...
    // Preorder: goc truoc, sau do trai, phai
    // -> load lai theo thu tu nay se tai tao dung cay can bang
    file << root->maThe << "|"
         << root->ho << "|"
         << root->ten << "|"
         << root->giotinh << "|"
