...
bool nhapChuoiTuDo(const char* thongBao, char* result, int maxLen, const char* initialValue) {
    ...
    while (true) {
        ...
        if (c == 13) { // ENTER
            while (len > 0 && result[len - 1] == ' ') len--; // Trim trailing space
            int start = 0;
            while (start < len && result[start] == ' ') start++;
            if (start > 0) { memmove(result, result + start, len - start); len -= start; } // Trim leading space (1 lan)
            result[len] = '\0';
            cout << endl;
            return true;
        } else if (c == 0 || c == 224) { // Phim chuc nang
...
static void veLaiFormInput(int x, int y, const char* buffer, int len, int vt, int maxLen) {
    gotoxy(x, y);
    // Thay vi clear bang string(maxLen, ' ') gay tran dong, chi in buffer va them khoang trang de xoa ky tu thua khi backspace
    cout << buffer << "   "; 
    gotoxy(x + vt, y);
}
